...
const LIFESTYLE = {
  ...
  sugar: {
    label: "Переел сахара", emoji: "🍭",
    keywords: ["наелся сладкого","наелась сладкого","переел сахара","переела сахара","сахарный скачок","объелся десерта","заедал сладким","сладкое и теперь вялый","сахарный отходняк","много конфет"],
    effect: "После порции быстрых углеводов сахар в крови подскакивает, а затем резко падает — мозгу не хватает топлива: приходят сонливость, вялость, раздражительность и «туман». Это гликемические качели.",
    tips: ["Добавьте к сладкому белок/клетчатку или пройдитесь 10–15 мин — это сглаживает скачок глюкозы.", "В следующий раз замените быстрые сахара на медленные (цельные продукты) — энергия ровнее."]
  },
  skipped_meal: {
    label: "Пропустил еду / голоден", emoji: "🍽️",
    keywords: ["весь день не ел","весь день не ела","пропустил обед","пропустила обед","давно не ел ничего","давно не ела ничего","забыл поесть","забыла поесть","голодал весь день","на голодный желудок мутит"],
    effect: "Долгий перерыв без еды роняет глюкозу — главное топливо мозга: падают концентрация и настроение, растёт раздражительность, труднее принимать решения (у префронтальной коры меньше энергии на самоконтроль).",
    tips: ["Поешьте сбалансированно (белок + сложные углеводы) — восстановит топливо ровно, без нового скачка.", "Не принимайте важных решений и не выясняйте отношения на голодный желудок."]
  },
  low_sunlight: {
    label: "Мало солнца", emoji: "☀️",
    keywords: ["мало солнца","не хватает света","давно не было солнца","весь день в помещении без света","зимой темно","пасмурно и вялый","нет дневного света","мало гулял на свету"],
    effect: "Недостаток яркого дневного света сбивает циркадные ритмы и снижает серотонин — отсюда вялость, тяга к углеводам и подавленность (механизм «зимней хандры» и сезонной депрессии).",
    tips: ["Ловите дневной свет утром 15–30 мин — это самый мощный сигнал для ритма и настроения.", "В тёмный сезон помогают яркая световая лампа и движение в светлое время."]
  },
  jetlag: {
    label: "Джетлаг / сбитый режим", emoji: "✈️",
    keywords: ["джетлаг","перелёт и всё сбилось","поменялись часовые пояса","сбитый режим сна","прилетел и не могу спать","прилетела и не могу спать","разница во времени","часовые пояса сбили"],
    effect: "При смене поясов внутренние часы (супрахиазмальное ядро) рассинхронизируются с внешним временем: сон, бодрость, аппетит и настроение «плавают», пока мозг не подстроится (примерно сутки на час сдвига).",
    tips: ["Сразу живите по местному времени и ловите утренний свет в точке прилёта — так часы перестраиваются быстрее всего.", "Избегайте яркого экрана ночью и кофеина во второй половине дня на новом месте."]
  },
  overtraining: {
    label: "Перетренировался", emoji: "🏋️",
    keywords: ["перетренировался","перетренировалась","выложился на тренировке","выложилась на тренировке","перегрузился в зале","переусердствовал со спортом","разбит после тренировки","загнал себя нагрузкой"],
    effect: "Чрезмерная или частая нагрузка без восстановления держит кортизол высоким и истощает нервную систему: приходят усталость, раздражительность, плохой сон и падение мотивации — это перетренированность, а не лень.",
    tips: ["Дайте телу восстановиться: сон, еда, день-два лёгкой активности вместо интенсива.", "Чередуйте тяжёлые и лёгкие дни — прогресс растёт в отдыхе, а не только в нагрузке."]
  },
  nicotine: {
    label: "Никотин / вейп", emoji: "🚬",
    keywords: ["много курил","много курила","весь день курю","вейп весь день","затягиваюсь весь день","много никотина","накурился","накурилась","часто парю"],
    effect: "Никотин быстро поднимает дофамин и бдительность через ацетилхолиновые рецепторы — но коротко; спад тянет к новой дозе. Так формируются качели «взбодрился → раздражение и тяга», а фоновая тревожность растёт.",
    tips: ["Заметьте: «успокоение» от сигареты — это снятие абстиненции, а не польза; фоновая тревога от никотина выше.", "Разрывайте связку «триггер → затяжка» (кофе, стресс) другими ритуалами: дыхание, вода, короткая прогулка."]
  },
  coffee_empty: {
    label: "Кофе натощак", emoji: "🥤",
    keywords: ["кофе натощак","выпил кофе на пустой желудок","выпила кофе на пустой желудок","кофе на голодный желудок","с утра только кофе","кофе без еды и трясёт"],
    effect: "Кофе на пустой желудок резче поднимает кортизол и норадреналин — отсюда дрожь, тревожность и сосание под ложечкой, особенно если это первый «приём» за день вместо еды.",
    tips: ["Пейте кофе после или во время еды — эффект мягче, без «дёрганности».", "Отложите первую чашку на 60–90 мин после пробуждения: утренний кортизол и так высок, кофе на его пике даёт перевозбуждение."]
  }
};
...
const I18N = {
  ru: {
    ...
    tab_brain: "Мозг", tab_journey: "Погружение", tab_programs: "Программы", tab_updates: "Открытия", install: "⤓ Установить",
    help_btn: "❔ Как пользоваться", ob_next: "Далее →", ob_back: "← Назад", ob_skip: "Пропустить", ob_done: "Начать 🚀",
  ...
  en: {
    ...
    tab_brain: "Brain", tab_journey: "Deep dive", tab_programs: "Programs", tab_updates: "Discoveries", install: "⤓ Install",
    help_btn: "❔ How to use", ob_next: "Next →", ob_back: "← Back", ob_skip: "Skip", ob_done: "Start 🚀",
